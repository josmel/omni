...
class Application_Model_DbTable_Project extends Core_Db_Table
{
    protected $_name = 'tproyecto';
    protected $_primary = "codproy";
    const NAMETABLE='tproyecto';
    ...
    static function populate($params) {
        ...
        if(isset($params['slug'])) $data['slug'] = $params['slug'];
    ...
    {
        return array("idbanner", "nombre", "DATE_FORMAT(fechainicio, '%d/%m/%Y')", "DATE_FORMAT(fechafin, '%d/%m/%Y')", "IF(vchestado LIKE 'A', 'Activo', 'Inactivo')");
    }
